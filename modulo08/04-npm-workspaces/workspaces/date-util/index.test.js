...
{
    const expected = '22/06/1990'
    const format = 'dd/mm/yyyy'
    const date = new Date('1990-06-22')
    const result = DateUtil.formatDate(date, format)
    deepStrictEqual(result, expected)
}

{
    const expected = '1990-06-22'
...

{
    ...
    const expected = {
        error: `the date ${data.value} is not in the format ${data.format}`
    }
...
{
    const data = {
        value: '1 9 9   0/0 2/0 1',
        format: 'yyyy/mm/dd'
    }

    const expectedFormat = 'dd-mm-yyyy'

    const expected = '01-02-1990'
    const result = DateUtil.formatString(data.value, data.format, expectedFormat)
    deepStrictEqual(result, expected)
}
