import StringUtil from '@henrique-pignatari/string-util'
...
export default class DateUtil {
    ...
    static formatString(dateStr, currentFormat, expectedFormat) {
        ...
        const clearDate = StringUtil
            .removeEmptySpaces(dateStr)

        if (!clearDate.match(toDateExp)) {
            return {
                error: `the date ${dateStr} is not in the format ${currentFormat}`
            }
        }


        const dateISOStr = clearDate.replace(toDateExp, '$<year>-$<month>-$<day>')
